...
def get_json_report():
    """
    :return: json report (as Python dictionary)
    """
    return json_templates.REPORT
...
def add_test_case(test_report, test_case):
    ...
    test['exception'] = test_case.excep_type
...
def add_grade_data_to_json(scores, score_weights, coverage_weights):
    """
    ScoreWeights, Scores -> NoneType
    Add grade data and custom weighting data to test report
    :param scores: overall, testing and code coverage scores
    ...
    test_report['custom']['coverageMethodWeight'] = round(coverage_weights.method_weight*100, 1)
    test_report['custom']['coverageLineWeight'] = round(coverage_weights.line_weight*100, 1)
    test_report['custom']['coverageBranchWeight'] = round(coverage_weights.branch_weight*100, 1)
