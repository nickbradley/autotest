...
export interface Result {
  ...
  user: string;
  report: object;
  studentBuildFailed: string;
  studentBuildMsg: string;
  deliverableBuildFailed: string;
  deliverableBuildMsg: string;
  deliverableRuntimeMsg: string;
  deliverableRuntimeError: string;
  commit: string;
  committer: string;
  timestamp: number;
  container: object;
...
export default class ResultRecord {
  ...
  public getGradeSummary(): any {
    try {
      ...
      Log.error(`ResultRecord::getGradeSummary Error Converting TestRecord to GradeView format: ${err}`)
    }
    ...
    try {
      ...
      Log.error(`ResultRecord::getGradeSummary Error Converting GradeView to GradeSummary format: ${err}.`)
    }
  }
  ...
  public convertGradeViewToGradeSummary(gradeView) {
    try {
      ...
      Log.error(`ResultRecord::convertGradeViewToGradeSummary ERROR Could not convert to GradeView: ${err}`);
    }
  }
}
